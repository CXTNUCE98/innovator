...
const profilesNotId = ref(teamOfExperts.value.filter((item) => item.id !== idExpert.value ))
const extendedBlogs = computed(() => {
    return [
        ...profilesNotId.value.slice(-visibleCourses.value),
        ...profilesNotId.value,
        ...profilesNotId.value.slice(0, visibleCourses.value),
    ];
});
...
const next = () => {
    ...
    // Check if at end
    const totalRealCourses = teamOfExperts.value?.length;
...
const handleTransitionEnd = () => {
    ...
    const total = teamOfExperts.value?.length;
...
<template>
    ...
    <div class="max-w-6xl mx-auto px-0 md:px-4 py-4 md:py-12">
        ...
        <div class="overflow-hidden" ref="carouselRef" @mouseenter="handleMouseEnter" @mouseleave="handleMouseLeave"
            ...
            <div class="flex py-2 gap-2 md:gap-6"
                :class="isTransitioning ? 'transition-transform duration-500 ease-in-out' : ''" :style="{
                ...
                <div v-for="(item, index) in extendedBlogs" :key="index" class="box-border"
                    ...
                    <div class="bg-neutral-50 rounded-2xl shadow-md p-16px flex flex-col gap-4 h-full text-center">
                        <img class="w-full h-full object-cover rounded-lg" :src="item.img" alt="img">
                        <div @click="handleRedirect(item.id)" class="text-#2A2A2A text-18px font-bold cursor-pointer">
                            {{ item.gender === 'male' ? 'Mr.' :'Mrs.' }} {{ item.name }}
                            <div class="text-#7F7F7F text-xs px-4 py-2 rounded-lg">{{
                                item.duty }}</div>
                        </div>
                    </div>
                </div>
